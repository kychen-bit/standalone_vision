...
现在只确认以下事情：相机能取流、ROI 正确、六个 Mask 正确、轮廓和全局中心正确、帧数与时间稳定门、
...
→ 当前目标颜色 cv2.inRange
→ 轻量开/闭运算
→ cv2.findContours → 外轮廓凸包
→ 面积、宽高比和贴边过滤
→ 选择面积最大的有效轮廓
→ cv2.moments 计算中心
→ 加回 ROI 左上角得到原图坐标
→ 至少8帧、至少250ms且中心位移不超过阈值
...
- 未同时满足 8 帧和 250 ms：`UNSTABLE`；
- 连续中心位移均不超过 6 px，且同时满足帧数与时间：`STABLE`；
- 中心突然跳动：重新从一帧累计；
- 目标消失：立即 `LOST`。
...
旧 `pickup` 速度/安全区/最终复检门控在配置中 `enabled=false`，协调器 `PICK` 使用帧数+时间
